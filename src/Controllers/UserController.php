...
class UserController extends BaseController{
  ...
  { 
    if ($this->getMethod() == 'get' && $endPoint == $this->getRoute()) {
      ...
      if(empty($email) || empty($password)){  
       return ResponseHttp::status400('Todos los campos son necesarios');
      }else if(!self::validateEmail($email)){
        return ResponseHttp::status400('Formato de correo invalido');
      }else{
  ...
  final public function postNewUser(string $endPoint)
  {
    ...
    $validator->setMessages([
      ...
      'dni'             => 'Dni debe contener solo números',
      'email'           => 'Email o formáto inválido',
      'rol'             => 'Rol numérico (1, 2 ó 3)',
      'password'        => 'Passwor alfanumérico mínimo 8 caracteres',
      'confirmPassword' => 'El password no coinciden' 
    ]);
    ...
    $validation = $validator->validate($this->getParam(), [
        ...
        'rol'                => 'required|numeric|min:1|regex:/^[123]+$/',
    ...
    if ($validation->fails()) {            
        ...
        echo json_encode(UserModel::postNewUser());
    }              
